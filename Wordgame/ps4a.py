...
def getWordScore(word, n):
    ...
    """
    score = 0
    #assert len(word)<=n , 'Word length cannot be greater than Hand size'
    word = str(word)
    for letter in word:
        if letter in 'abcdefghijklomnpqrstuvwxyz':
...
def isValidWord(word, hand, wordList):
    ...
    for letter in word:
        if letter not in hand.keys():
            return False
        elif counter[letter]> hand[letter]:
            return False
    return True    
    ...
    # As long as there are still letters left in the hand:
        ...
        # If the input is a single period:
            ...
            if not isValidWord(input_word,hand,wordList):
                ...
                wordScore = getWordScore(input_word,n)
                totalScore += wordScore
                hand = updateHand(hand,input_word)
                #print hand
                print '"' + input_word + '" earned ' + str(wordScore) + ' points. Total: ' + str(totalScore) + ' points.' 
                print ''
        if not hand:
            print 'hand is empty'
            break
